...
namespace fp {
    ...
    public:
        ...
        static std::string ordering_to_string(const ordered_query& q, std::true_type) {
            return stringutils::concatenate(", ", to_string(q._column), " ", impl::ordered_query::ORDERINGS[static_cast<int>(q._order)]);
        }

        static std::string ordering_to_string(const ordered_query& q, std::false_type) {
            return stringutils::concatenate(" ORDER BY ", to_string(q._column), " ", impl::ordered_query::ORDERINGS[static_cast<int>(q._order)]);
        }

        friend std::string to_string(const ordered_query& q) {
            using std::to_string;
            std::string ordering = ordering_to_string(q, is_ordered_query<TQuery>{});
