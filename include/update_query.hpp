...
struct is_update_query<update_query<TUpdate...> >
: mpl::all_<impl::is_update_modifier<TUpdate>...>
{ };

template<typename... TUpdate>
struct is_query<update_query<TUpdate...> >
: is_update_query<update_query<TUpdate...> >
{ };
...
public:
  ...
  >
  friend TRecord& update(TRecord& rec, const update_query& self)
  {
    impl::update_applier<TUpdate...> eval = call_constructor<impl::update_applier<TUpdate...>>(self.updates_);
    eval(rec);
    return rec;
  }
  ...
  template<
    ...
    typename = mpl::enable_if_t<is_record<TRecord> >
  >
  friend TContainer update(TContainer cont, const update_query& self)
  {
  ...
  friend std::string to_string(const update_query& self, Formatter& formatter)
  {
    std::string(*build_fn)(TUpdate..., Formatter&) = &impl::update_query_impl<TUpdate...>::build_update_query;
    return call_function(build_fn, self.updates_, formatter);
  }
...
constexpr inline impl::update_modifiers::modifier<
          ...
          meta::if_<
            is_column<TRight>,
            impl::update_modifiers::column_getter<TRight>,
            impl::update_modifiers::value_getter<TLeft>
          > >
set(TLeft l, TRight r) {
  using getter_type = meta::if_<
              is_column<TRight>,
              impl::update_modifiers::column_getter<TRight>,
              impl::update_modifiers::value_getter<TLeft>
            >;
...
#define DECLARE_BIN_OP_UPDATE(name, suffix) \
  ...
  constexpr inline impl::update_modifiers::modifier< \
            ...
            meta::if_< \
              is_column<TRight>, \
              impl::update_modifiers::column ## suffix <TLeft, TRight>, \
              impl::update_modifiers::value ## suffix <TLeft> \
            > > \
