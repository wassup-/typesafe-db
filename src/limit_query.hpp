...
namespace fp {
    ...
    inline bool evaluate(record<TDescriptor, Fs...> const & r, limit_query<TQuery> const & q) {
        return q.evaluate(r);
    }

    template<typename TQuery, typename TDescriptor, int... Fs>
    inline typename TQuery::result_type select(record<TDescriptor, Fs...> const & r, limit_query<TQuery> const & q) {
        return q.select(r);
    }
}
