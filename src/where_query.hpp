...
namespace fp {
    ...
    inline typename std::enable_if<(sizeof...(TClauses) > 0), where_query<TDescriptor, TClauses..., TClause> >::type operator+(where_query<TDescriptor, TClauses...> q, TClause c) {
    ...
    template<typename TDescriptor, typename... TClauses, typename TClause>
    inline typename std::enable_if<(sizeof...(TClauses) == 0), where_query<TDescriptor, TClauses..., TClause> >::type operator+(where_query<TDescriptor, TClauses...>, TClause c) {
    ...
    inline auto evaluate(record<TDescriptor, Fs...> const & rec, where_query<TDescriptor, TClauses...> const & qry) -> decltype(qry.evaluate(rec)) {
        return qry.evaluate(rec);
    }

    template<typename TDescriptor, typename... TClauses, int... Fs>
    inline auto evaluate(std::vector<record<TDescriptor, Fs...> > const & recs, where_query<TDescriptor, TClauses...> const & qry) -> decltype(qry.evaluate(recs)) {
